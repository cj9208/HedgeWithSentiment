...
def spline_map(df, N=10, d=1, volatility=False, TYPE='C', sentiment=False):
    ...
    if volatility:
        if TYPE == 'C':    
            ...
            dfret['volatility'] = spline_generate(df['delta'], N, d, 'delta', left=-1, right=0)
        else:
...
class NianHedge(BaseEstimator, ClassifierMixin):
    def __init__(self, lamb=1, kernel='spline', N=10, d=1, loss_d=2, volatility=False, TYPE='C', 
    ...
    def fit(self, df_feature, dV, dS):
        ...
        if self.kernel == 'spline':
            df_feature = spline_map(df_feature, N=self.N, d=self.d, volatility=self.volatility, TYPE=self.TYPE, sentiment=self.sentiment)
...
class NianDeltaBucket():
    def __init__(self, para, TYPE='C'):
        ...
        for i in range(1,9+1):
            base_model[i] = NianHedge(lamb=self.para['lamb'], N=self.para['N'], 
                    d=self.para['d'], loss_d=self.para['loss_d'], volatility=self.para['volatility'], 
                    TYPE = TYPE, multiple=self.para['multiple'], debug=self.para['debug'])   
    ...
    def fit(self, df_feature, dV, dS):
        ...
        for i in range(1,9+1):
            ...
            self.classifier[i].fit(df_part, dV_part, dS_part)
    ...
    def predict(self, df_feature):
        ...
        for i in range(1,9+1):
            ...
            valid_delta = self.classifier[i].predict(df_feature)


            print(type(valid_delta))
